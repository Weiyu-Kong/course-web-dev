  ...
  try {
    ...
    results.innerHTML = `
      <div class="section-heading">
        <div><span>STEP 2</span><h2>${escapeHtml(data.criteria.from)} → ${escapeHtml(data.criteria.to)}</h2></div>
...
function trainCard(train) {
  return `<article class="train-card">
    ...
    <div><small>Departure time</small><b>${escapeHtml(train.departureTime)}</b><span>${escapeHtml(train.departureStation)}</span></div>
    <div class="route-line"><span>${train.durationMinutes} min</span><i></i></div>
    <div class="arrival"><small>Arrival time</small><b>${escapeHtml(train.arrivalTime)}</b><span>${escapeHtml(train.arrivalStation)}</span></div>
